...
The first release is a solo desktop game. The Norman player chooses strategies, facings, movement, fire, and melee. The Saxon AI plays its own segments. The phase button advances the turn, and the event log records dice and results. The game opens on a tapestry-inspired title screen; the battlefield appears after New Game or Load Game. The larger parchment-colored battle panel and reference charts scale with the display. In a battle, Escape opens the menu for Resume, New Game, Save Game, and Load Game. Save files are versioned JSON in Unity's `Application.persistentDataPath/Saves` folder.
...
The bundled PDF is available through **Rulebook PDF**. The Melee, Missile, and Morale reference tabs render readable tables from the same rule data used by combat and rally. The SVG map is converted into `Assets/Resources/Art/Map/hex_map.png`, and its terrain data into `Assets/Resources/Data/Map.json`, by `../Tools/generate_assets.py`. That script uses `rsvg-convert` and Python's Pillow package to rasterize textured ridges from the SVG's marked edges. It leaves the SVG untouched and does not use Inkscape. The map texture and data are checked in, so rebuilding them is unnecessary for normal Unity use.
