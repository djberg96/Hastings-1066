...
The first release is a solo desktop game. The Norman player chooses strategies, facings, movement, fire, and melee. The Saxon AI plays its own segments. The phase button advances the turn, and the event log records dice and results. The game opens on a tapestry-inspired title screen; the battlefield appears after New Game or Load Game. In a battle, Escape opens the menu for Resume, New Game, Save Game, and Load Game. Save files are versioned JSON in Unity's `Application.persistentDataPath/Saves` folder.
